...
#Region "Methods"
    ...
    Public Shared Function GetTVSeasons(ByVal sPath As String, ByVal ShowID As Long, ByVal MinEp As Integer) As List(Of EpisodeItem)
        ...
        For Each rShow As Settings.regexp In Master.eSettings.TVShowMatching
            ...
            'If retEpisodeItemsList.Count > 0 Then
            '    'clean entries
            ...
            '        For i As Integer = retEpisodeItemsList.Count - 1 To 0 Step -1
            '            'remove any unknown season or seasons where all episodes are unknown
            '            If retSeason(i).Season < 0 OrElse retSeason(i).Episodes.Where(Function(e) e < 0).Count = retSeason(i).Episodes.Count Then retEpisodeItemsList.Remove(retEpisodeItemsList(i))
            '        Next
            '    End If

            '    'if we still have something left, lets use it
            '    If retEpisodeItemsList.Count > 0 Then Return retEpisodeItemsList
            'End If
        Next

        ''nothing found
        'eItem = New EpisodeItemList
        'cSeason.Season = -1
        'cSeason.Episodes.Add(MinEp)
        'retEpisodeItemsList.Add(eItem)
