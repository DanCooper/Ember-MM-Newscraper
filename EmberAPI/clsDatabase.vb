...
#Region "Methods"
    ...
    Public Sub Clean(ByVal cleanMovies As Boolean,
        ...
        _Logger.Info("Cleaning videodatabase started")

        Using sqlTransaction As SQLiteTransaction = _MyvideosDBConn.BeginTransaction()
            If cleanMovies Then
                _Logger.Info("Cleaning movies started")
                ...
                Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                        ...
                        While sqlReader.Read
                            ...
                            If tSource IsNot Nothing Then
                                ...
                                If Not nFileItem.bIsOnline OrElse Not FileUtils.Common.IsValidFile(nFileItem.FileInfo, Enums.ContentType.Movie) Then
                                    Master.DB.Remove_Movie(CLng(sqlReader("idMovie")), True)
                                End If
                            Else
                    ...
                    End Using
                End Using
                _Logger.Info("Cleaning movies done")
            End If

            If cleanMovieSets Then
                _Logger.Info("Cleaning moviesets started")
                Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                    ...
                    Using SQLreader As SQLiteDataReader = sqlCommand.ExecuteReader()
                        While SQLreader.Read
                            If Convert.ToInt64(SQLreader("mCount")) = 0 Then
                                Master.DB.Remove_MovieSet(CLng(SQLreader("idSet")), True)
                            End If
                        End While
                    End Using
                End Using
                _Logger.Info("Cleaning moviesets done")
            End If

            If cleanTVShows Then
                _Logger.Info("Cleaning tv shows started")
                Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                    If idSource = -1 Then
                    ...
                    Using sqlReader As SQLiteDataReader = sqlCommand.ExecuteReader()
                        While sqlReader.Read
                            ...
                            If Not nFileItem.bIsOnline OrElse Not FileUtils.Common.IsValidFile(nFileItem.FileInfo, Enums.ContentType.TVEpisode) Then
                                Master.DB.Remove_TVEpisode(CLng(sqlReader("idEpisode")), False, False, True)
                            End If
                        End While
                    End Using

                    _Logger.Info("Removing tvshows without local episodes")
                    ...
                    Using sqlReader As SQLiteDataReader = sqlCommand.ExecuteReader()
                        While sqlReader.Read
                            Master.DB.Remove_TVShow(CLng(sqlReader("idShow")), True)
                        End While
                    End Using
                    _Logger.Info("Removing seasons with no more existing tvshows")
                    sqlCommand.CommandText = "DELETE FROM season WHERE idShow NOT IN (SELECT idShow FROM tvshow);"
                    sqlCommand.ExecuteNonQuery()
                    _Logger.Info("Removing episodes with no more existing tvshows")
                    sqlCommand.CommandText = "DELETE FROM episode WHERE idShow NOT IN (SELECT idShow FROM tvshow);"
                    sqlCommand.ExecuteNonQuery()
                    _Logger.Info("Removing episodes with orphaned paths")
                    ...
                    sqlCommand.ExecuteNonQuery()
                    _Logger.Info("Removing orphaned paths")
                ...
                _Logger.Info("Removing seasons with no more existing episodes")
                Remove_Empty_TVSeasons(-1, True)
                _Logger.Info("Cleaning tv shows done")
            End If

            'global cleaning
            _Logger.Info("Cleaning global tables started")
            Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                'clean all link tables
                'actor_link
                _Logger.Info("Cleaning actor_link table")
                ...
                'certification_link
                _Logger.Info("Cleaning certification_link table")
                ...
                'country_link
                _Logger.Info("Cleaning country_link table")
                ...
                'creator_link
                _Logger.Info("Cleaning creator_link table")
                CleanLinkTable("creator_link", "tvshow", "idShow", Enums.ContentType.TVShow)
                'director_link
                _Logger.Info("Cleaning director_link table")
                ...
                'genre_link
                _Logger.Info("Cleaning genre_link table")
                ...
                'writer_link
                _Logger.Info("Cleaning writer_link table")
                ...
                'certification
                _Logger.Info("Cleaning certification table")
                CleanMainTable("certification", "certification_link", "idCertification")
                'country
                _Logger.Info("Cleaning country table")
                CleanMainTable("country", "country_link", "idCountry")
                'genre
                _Logger.Info("Cleaning genre table")
                CleanMainTable("genre", "genre_link", "idGenre")
                'person
                _Logger.Info("Cleaning person table")
                CleanPersonTable()
                'studio
                _Logger.Info("Cleaning studio table")
                CleanMainTable("studio", "studio_link", "idStudio")
            End Using
            _Logger.Info("Cleaning global tables done")

            sqlTransaction.Commit()
        End Using

        _Logger.Info("Cleaning videodatabase done")

        ' Housekeeping - consolidate and pack database using vacuum command http://www.sqlite.org/lang_vacuum.html
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
            _Logger.Info("Rebulding videodatabase started")
            sqlCommand.CommandText = "VACUUM;"
            sqlCommand.ExecuteNonQuery()
            _Logger.Info("Rebulding videodatabase done")
        End Using
    End Sub
    ...
    Private Function GetMoviesetsForMovie(ByVal idMovie As Long) As List(Of MediaContainers.SetDetails)
        ...
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
            sqlCommand.CommandText = String.Concat("SELECT a.idSet, b.plot, b.title FROM movieset_link ",
    ...
    Public Sub Cleanup_Genres()
        _Logger.Info("[Database] [Cleanup_Genres] Started")
        ...
        Using sqlTransaction As SQLiteTransaction = _MyvideosDBConn.BeginTransaction()
            _Logger.Info("[Database] [Cleanup_Genres] Process all Movies")
            ...
            For Each lMovieID In lstMovies
                ...
                If tmpDBElement.IsOnline Then
                    ...
                    End If
                Else
                    _Logger.Warn(String.Concat("[Database] [Cleanup_Genres] Skip Movie (not online): ", tmpDBElement.FileItem.FullPath))
                End If
            Next

            'Process all TVShows, which are assigned to a genre
            _Logger.Info("[Database] [Cleanup_Genres] Process all TVShows")
            For Each lTVShowID In lstTVShows
                ...
                If tmpDBElement.IsOnline Then
                    ...
                    End If
                Else
                    _Logger.Warn(String.Concat("[Database] [Cleanup_Genres] Skip TV Show (not online): ", tmpDBElement.ShowPath))
                End If
            Next

            'Cleanup genre table
            Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                _Logger.Info("[Database] [Cleanup_Genres] Cleaning genre table")
            ...
            sqlTransaction.Commit()
        End Using
        _Logger.Info("[Database] [Cleanup_Genres] Done")
    End Sub
    ''' <summary>
    ...
    '''                        will trow exception
    ''' </remarks>
    Public Sub ClearNew(ByVal ContentType As Enums.ContentType)
    ...
    Protected Sub CloseDatabase(ByRef connection As SQLiteConnection)
        ...
        Catch ex As Exception
            _Logger.Error(ex, New StackFrame().GetMethod().Name & Convert.ToChar(Windows.Forms.Keys.Tab) & "There was a problem closing the media database.")
        Finally
            connection.Dispose()
        End Try
    End Sub
    ...
    Public Function Connect_MyVideos() As Boolean
        ...
        Catch ex As Exception
            _Logger.Error(ex, New StackFrame().GetMethod().Name & Convert.ToChar(Windows.Forms.Keys.Tab) & "Unable to open media database connection.")
        End Try
        ...
        Try
            ...
            End If
        Catch ex As Exception
            _Logger.Error(ex, New StackFrame().GetMethod().Name & Convert.ToChar(Windows.Forms.Keys.Tab) & "Error creating database")
            Close_MyVideos()
    ...
    Public Function AddTVShowInfoToDBElement(ByVal episode As DBElement, Optional ByVal tvshow As DBElement = Nothing) As DBElement
        Dim tmpTVDBShow As DBElement

        If tvshow Is Nothing OrElse tvshow.MainDetails Is Nothing Then
            tmpTVDBShow = Load_TVShow(episode.ShowID, False, False)
        Else
            tmpTVDBShow = tvshow
        End If

        episode.EpisodeSorting = tmpTVDBShow.EpisodeSorting
        episode.EpisodeOrdering = tmpTVDBShow.EpisodeOrdering
        episode.Language = tmpTVDBShow.Language
        episode.ShowID = tmpTVDBShow.ShowID
        episode.ShowPath = tmpTVDBShow.ShowPath
        episode.Source = tmpTVDBShow.Source
        episode.TVShowDetails = tmpTVDBShow.MainDetails
        Return episode
    End Function
    ...
    Public Function GetAllMovieSetDetails() As List(Of MediaContainers.SetDetails)
        ...
        For Each nSet In Load_AllMoviesets()
            nList.Add(New MediaContainers.SetDetails With {
                      .ID = nSet.ID,
                      .Plot = nSet.MainDetails.Plot,
                      .Title = nSet.MainDetails.Title,
                      .TMDbId = nSet.MainDetails.UniqueIDs.TMDbId
                      })
        Next
        Return nList
    End Function
    ...
    Public Sub LoadAllGenres()
        ...
        For Each tGenre As String In gList
            Dim gMapping As GenreMapping = APIXML.GenreMapping.Mappings.FirstOrDefault(Function(f) f.SearchString = tGenre)
            If gMapping Is Nothing Then
                ...
                If gProperty Is Nothing Then
                    APIXML.GenreMapping.Genres.Add(New GenreProperty With {.IsNew = False, .Name = tGenre})
                End If
                'add a new mapping if tGenre is not in the MappingTable
                APIXML.GenreMapping.Mappings.Add(New GenreMapping With {.isNew = False, .MappedTo = New List(Of String) From {tGenre}, .SearchString = tGenre})
            End If
        Next
    End Sub
    ...
    Public Function GetTVSeasonIDFromEpisode(ByVal dbElement As DBElement) As Long
        ...
        If dbElement.MainDetails IsNot Nothing Then
            Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                sqlCommand.CommandText = String.Format("SELECT idSeason FROM season WHERE idShow={0} AND season={1};", dbElement.ShowID, dbElement.MainDetails.Season)
                Using SQLreader As SQLiteDataReader = sqlCommand.ExecuteReader()
                    If SQLreader.HasRows Then
                        SQLreader.Read()
    ...
    Public Function DeleteView(ByVal viewName As String) As Boolean
        ...
        If viewName = "episodelist" OrElse
            ...
            _Logger.Error("It's not allowed to delete a default list")
            Return False
        End If
    ...
    Public Function GetViewMediaCount(ByVal viewName As String, Optional episodesByView As Boolean = False) As Integer
        ...
        If Not String.IsNullOrEmpty(viewName) Then
            Try
                ...
                End If
            Catch ex As Exception
                _Logger.Error(String.Format("SQL error in ""{0}"": {1}", viewName, ex.Message))
                Return -1
            End Try
        Else
            Return iCount
    ...
    Public Function GetSources_Movie(Optional ByVal filter As SmartFilter.Filter = Nothing) As DataTable
        Dim nDataTable As New DataTable
        If filter Is Nothing Then
            filter = New SmartFilter.Filter(Enums.ContentType.Source_Movie)
        End If
        FillDataTable(nDataTable, filter)
        Return nDataTable
    End Function

    Public Function GetSources_TVShow(Optional ByVal filter As SmartFilter.Filter = Nothing) As DataTable
        Dim nDataTable As New DataTable
        If filter Is Nothing Then
            filter = New SmartFilter.Filter(Enums.ContentType.Source_TVShow)
        End If
        FillDataTable(nDataTable, filter)
        Return nDataTable
    End Function
    ...
    Public Function Load_Movie(ByVal idMovie As Long) As DBElement
        Dim dbElement As New DBElement(Enums.ContentType.Movie) With {
            .ID = idMovie
        }
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
            ...
            Using sqlReader As SQLiteDataReader = sqlCommand.ExecuteReader()
                If sqlReader.HasRows Then
                    ...
                    dbElement.IsNew = Convert.ToBoolean(sqlReader("new"))
        ...
        'Actors
        dbElement.MainDetails.Actors = GetActorsForItem(dbElement.ID, dbElement.ContentType)
        ...
        'Certifications
        dbElement.MainDetails.Certifications = GetCertificationsForItem(dbElement.ID, dbElement.ContentType)

        'Countries
        dbElement.MainDetails.Countries = GetCountriesForItem(dbElement.ID, dbElement.ContentType)

        'Credits
        dbElement.MainDetails.Credits = GetWritersForItem(dbElement.ID, dbElement.ContentType)

        'Directors
        dbElement.MainDetails.Directors = GetDirectorsForItem(dbElement.ID, dbElement.ContentType)

        'External subtitles
        dbElement.Subtitles = GetExternalSubtitlesForFileItem(dbElement.FileItem)

        'FileInfo
        dbElement.MainDetails.FileInfo = GetFileInfoForFileItem(dbElement.FileItem)

        'Genres
        dbElement.MainDetails.Genres = GetGenresForItem(dbElement.ID, dbElement.ContentType)

        'Moviesets
        dbElement.MainDetails.Sets.AddRange(GetMoviesetsForMovie(dbElement.ID))

        'Ratings
        dbElement.MainDetails.Ratings = GetRatingsForItem(dbElement.ID, dbElement.ContentType)

        'Studios
        dbElement.MainDetails.Studios = GetStudiosForItem(dbElement.ID, dbElement.ContentType)

        'Tags
        dbElement.MainDetails.Tags = GetTagsForItem(dbElement.ID, dbElement.ContentType)

        'TV Show Links
        dbElement.MainDetails.ShowLinks = GetTVShowLinksForMovie(dbElement.ID, dbElement.ContentType)

        'UniqueIDs
        dbElement.MainDetails.UniqueIDs = GetUniqueIDsForItem(dbElement.ID, dbElement.ContentType)
    ...
    Public Function Load_Movieset(ByVal idSet As Long) As DBElement
        Dim dbElement As New DBElement(Enums.ContentType.Movieset) With {
            .ID = idSet
        }
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
    ...
    Public Function Load_TVEpisode(ByVal idEpisode As Long, ByVal withShow As Boolean) As DBElement
        Dim dbElement As New DBElement(Enums.ContentType.TVEpisode) With {
            .ID = idEpisode
        }
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
        ...
        'GuestStars
        dbElement.MainDetails.GuestStars = GetGuestStarsForItem(dbElement.ID, dbElement.ContentType)
    ...
    Public Function Load_TVSeason(ByVal idSeason As Long, ByVal withShow As Boolean, ByVal withEpisodes As Boolean) As DBElement
        Dim dbElement As New DBElement(Enums.ContentType.TVSeason) With {
            .ID = idSeason
        }
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
            sqlCommand.CommandText = String.Format("SELECT * FROM season WHERE idSeason={0};", dbElement.ID)
            Using sqlReader As SQLiteDataReader = sqlCommand.ExecuteReader
                If sqlReader.HasRows Then
                    sqlReader.Read()
                    dbElement.IsLocked = Convert.ToBoolean(sqlReader("locked"))
        ...
        If withEpisodes Then
            For Each tEpisode As DBElement In Load_AllTVEpisodes(dbElement.ShowID, withShow, dbElement.MainDetails.Season)
    ...
    Public Function Load_TVShow(ByVal idShow As Long, ByVal withSeasons As Boolean, ByVal withEpisodes As Boolean, Optional ByVal withMissingEpisodes As Boolean = False) As DBElement
        Dim dbElement As New DBElement(Enums.ContentType.TVShow) With {
            .ID = idShow,
            .ShowID = idShow
        }
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
        ...
        'Creators
        dbElement.MainDetails.Creators = GetCreatorsForItem(dbElement.ID, dbElement.ContentType)
        ...
        If withSeasons Then
            For Each tSeason As DBElement In Load_AllTVSeasons(dbElement.ID)
                ...
                dbElement.MainDetails.Seasons.Seasons.Add(tSeason.MainDetails)
            Next
    ...
    ''' <remarks></remarks>
    Private Sub Remove_Empty_TVSeasons(ByVal idShow As Long, ByVal batchMode As Boolean)
        Dim SQLtransaction As SQLiteTransaction = Nothing
    ...
    ''' <param name="batchMode">If <c>False</c>, the action is wrapped in a transaction</param>
    ''' <remarks></remarks>
    Private Sub Remove_Invalid_TVEpisodes(ByVal validEpisodes As List(Of DBElement), ByVal idShow As Long, ByVal batchMode As Boolean)
        Dim SQLtransaction As SQLiteTransaction = Nothing
            ...
            Using SQLreader As SQLiteDataReader = sqlCommand.ExecuteReader()
                While SQLreader.Read
                    If validEpisodes.Where(Function(f) f.ID = Convert.ToInt64(SQLreader("idEpisode"))).Count = 0 Then
                        Remove_TVEpisode(Convert.ToInt64(SQLreader("idEpisode")), True, False, True)
                    End If
                End While
            End Using
        End Using
    ...
    ''' <param name="batchMode">If <c>False</c>, the action is wrapped in a transaction</param>
    ''' <remarks></remarks>
    Private Sub Remove_Invalid_TVSeasons(ByVal validSeasons As List(Of DBElement), ByVal idShow As Long, ByVal batchMode As Boolean)
        Dim SQLtransaction As SQLiteTransaction = Nothing
            ...
            Using SQLreader As SQLiteDataReader = sqlCommand.ExecuteReader()
                While SQLreader.Read
                    If validSeasons.Where(Function(f) f.ID = Convert.ToInt64(SQLreader("idSeason"))).Count = 0 Then
                        Remove_TVSeason(Convert.ToInt64(SQLreader("idSeason")), True)
                    End If
                End While
            End Using
        End Using
    ...
    Public Sub Remove_Movie(ByVal idMovie As Long, ByVal batchMode As Boolean)
        If Not idMovie = -1 Then
            Dim nDBElement As DBElement = Load_Movie(idMovie)
            AddonsManager.Instance.RunGeneric(Enums.AddonEventType.Remove_Movie, Nothing, Nothing, False, nDBElement)
            ...
            Master.Events.SendEvent(Enums.AddonEventType.Remove_Movie, New List(Of Object)(New Object() {nDBElement.ID}))
        End If
    End Sub
    ''' <summary>
    ...
    ''' <param name="batchMode">Is this function already part of a transaction?</param>
    Public Sub Remove_MovieSet(ByVal idMovieset As Long, ByVal batchMode As Boolean)
        If Not idMovieset = -1 Then
            ...
            If lstMoviesToEdit.Count > 0 Then
                For Each movie In lstMoviesToEdit
                    movie.MainDetails.RemoveSet(idMovieset)
                    AddonsManager.Instance.RunGeneric(Enums.AddonEventType.BeforeEdit_Movie, Nothing, Nothing, False, movie)
                    AddonsManager.Instance.RunGeneric(Enums.AddonEventType.AfterEdit_Movie, Nothing, Nothing, False, movie)
                    Save_Movie(movie, batchMode, True, False, True, False)
                    Master.Events.SendEvent(Enums.AddonEventType.AfterEdit_Movie, New List(Of Object)(New Object() {movie.ID}))
                Next
            End If
    ...
    ''' <param name="batchMode">Is this function already part of a transaction?</param>
    Public Sub Remove_Tag(ByVal idTag As Long, ByVal batchMode As Boolean)
        If Not idTag = -1 Then
            Dim lstDBElement As New List(Of DBElement)
            ...
            If lstDBElement.Count > 0 Then
                For Each nDBElement In lstDBElement
                    nDBElement.MainDetails.Tags.Remove(strTagName)
    ...
    Public Function Remove_TVEpisode(ByVal idEpisode As Long, ByVal doForce As Boolean, ByVal doCleanSeasons As Boolean, ByVal batchMode As Boolean) As Boolean
        Dim sqlTransaction As SQLiteTransaction = Nothing
        Dim doesExist As Boolean = False
                ...
                While sqlReader.Read
                    Using sqlCommand_delete As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                        ...
                        If doForce OrElse doesExist Then
                            ...
                            If doCleanSeasons Then Master.DB.Remove_Empty_TVSeasons(CLng(sqlReader("idShow")), True)
                            bHasRemoved = True
    ...
    Public Sub Remove_TVEpisode(ByVal path As String, ByVal doForce As Boolean, ByVal batchMode As Boolean)
        Dim sqlTransaction As SQLiteTransaction = Nothing
        If Not batchMode Then sqlTransaction = _MyvideosDBConn.BeginTransaction()
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
            ...
            Using sqlReader As SQLiteDataReader = sqlCommand.ExecuteReader
                While sqlReader.Read
                    Remove_TVEpisode(CLng(sqlReader("idEpisode")), doForce, False, batchMode)
                End While
            End Using
        End Using
        If Not batchMode Then sqlTransaction.Commit()
    ...
    Public Sub Remove_TVSeason(ByVal idSeason As Long, ByVal batchMode As Boolean)
        Dim sqlTransaction As SQLiteTransaction = Nothing
        If Not batchMode Then sqlTransaction = _MyvideosDBConn.BeginTransaction()
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
    ...
    ''' <param name="batchMode">Is this function already part of a transaction?</param>
    ''' <returns>True if successful, false if deletion failed.</returns>
    Public Function Remove_TVSeason(ByVal idShow As Long, ByVal season As Integer, ByVal batchMode As Boolean) As Boolean
    ...
    ''' <param name="batchMode">Is this function already part of a transaction?</param>
    ''' <returns>True if successful, false if deletion failed.</returns>
    Public Function Remove_TVShow(ByVal idShow As Long, ByVal batchMode As Boolean) As Boolean
        Dim tvshowDB As DBElement = Load_TVShow_Full(idShow)
        AddonsManager.Instance.RunGeneric(Enums.AddonEventType.Remove_TVShow, Nothing, Nothing, False, tvshowDB)
        ...
        Try
            ...
            Master.Events.SendEvent(Enums.AddonEventType.Remove_TVShow, New List(Of Object)(New Object() {tvshowDB.ID}))
        Catch ex As Exception
            _Logger.Error(ex, New StackFrame().GetMethod().Name)
            Return False
        End Try
    ...
    Public Function Save_Movie(ByVal dbElement As DBElement, ByVal batchMode As Boolean, ByVal toNFO As Boolean, ByVal toDisk As Boolean, ByVal doSync As Boolean, ByVal forceFileCleanup As Boolean) As DBElement
        ...
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
            ...
            'DateAdded
            Try
                If Not Master.eSettings.Movie.SourceSettings.DateAddedIgnoreNfo AndAlso dbElement.MainDetails.DateAddedSpecified Then
            ...
            If toNFO Then NFO.Save(dbElement, forceFileCleanup)
            If toDisk Then
            ...
            If Not dbElement.IDSpecified Then
                If Master.eSettings.Movie.SourceSettings.MarkNewAsNew Then
                ...
                Using sqlReader As SQLiteDataReader = sqlCommand.ExecuteReader()
                    ...
                    Else
                        _Logger.Error("Something very wrong here: Save_Movie", dbElement.ToString)
                        dbElement.ID = -1
                        Return dbElement
                    End If
                End Using
            ...
            If dbElement.IDSpecified Then
                'Actors
                SetActorsForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.Actors)
                ...
                'Certifications
                SetCertificationsForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.Certifications)

                'Countries
                SetCountriesForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.Countries)

                'Directors
                SetDirectorsForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.Directors)

                'StreamDetails and external subtitles
                SetFileInfoForFileItem(dbElement.FileItem, dbElement.MainDetails.FileInfo)
                SetExternalSubtitlesForFileItem(dbElement.FileItem, dbElement.Subtitles)

                'Genres
                SetGenresForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.Genres)

                'Movieset
                SetMoviesetsForMovie(dbElement, dbElement.MainDetails.Sets)

                'Ratings
                SetRatingsForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.Ratings)

                'Studios
                SetStudiosForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.Studios)

                'Tags
                SetTagsForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.Tags)

                'TVShow Links
                SetTVShowLinksForMovie(dbElement)

                'UniqueIDs
                SetUniqueIDsForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.UniqueIDs)

                'Writers
                SetWritersForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.Credits)
            End If
        End Using
        ...
        If doSync Then
            AddonsManager.Instance.RunGeneric(Enums.AddonEventType.Sync_Movie, Nothing, Nothing, False, dbElement)
        End If

        Master.Events.SendEvent(Enums.AddonEventType.AfterEdit_Movie, New List(Of Object) From {dbElement.ID})
        Return dbElement
    End Function
    ''' <summary>
    ''' Saves all information from a Database.DBElement object to the database
    ...
    ''' <returns>Database.DBElement object</returns>
    Public Function Save_Movieset(ByVal dbElement As DBElement, ByVal bBatchMode As Boolean, ByVal bToNFO As Boolean, ByVal bToDisk As Boolean, ByVal bDoSync As Boolean, Optional ByVal forceFileCleanup As Boolean = False) As DBElement
        If dbElement.MainDetails Is Nothing Then Return dbElement
        ...
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
            ...
            If bToNFO Then NFO.Save(dbElement, forceFileCleanup)
            If bToDisk Then
            ...
            par_new.Value = Not dbElement.IDSpecified
            par_nfoPath.Value = dbElement.NfoPath
            par_plot.Value = dbElement.MainDetails.Plot
            ...
            If Not dbElement.IDSpecified Then
                ...
                Using rdrMovieSet As SQLiteDataReader = sqlCommand.ExecuteReader()
                    ...
                    Else
                        _Logger.Error("Something very wrong here: Save_MovieSet", dbElement.ToString, "Error")
                        dbElement.MainDetails.Title = "ERROR"
                        Return dbElement
                    End If
                End Using
            Else
        ...
        For Each tMovie In dbElement.MoviesInSet
            tMovie.DBMovie.MainDetails.AddSet(New MediaContainers.SetDetails With {
                                            ...
                                            .TMDbId = dbElement.MainDetails.UniqueIDs.TMDbId
                                            })
            ...
            Master.Events.SendEvent(Enums.AddonEventType.AfterEdit_Movie, New List(Of Object)(New Object() {tMovie.DBMovie.ID}))
        Next
            ...
            Using SQLreader As SQLiteDataReader = sqlCommand.ExecuteReader()
                While SQLreader.Read
                    ...
                    If rMovie Is Nothing Then
                        ...
                        tMovie.MainDetails.RemoveSet(dbElement.ID)
                        ...
                        Master.Events.SendEvent(Enums.AddonEventType.AfterEdit_Movie, New List(Of Object)(New Object() {tMovie.ID}))
                    End If
                End While
            End Using
        End Using
        ...
        AddonsManager.Instance.RunGeneric(Enums.AddonEventType.Sync_MovieSet, Nothing, Nothing, False, dbElement)
        ...
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
            ...
            If bIsNew Then
                Using rdrMovieTag As SQLiteDataReader = sqlCommand.ExecuteReader()
                    ...
                    Else
                        _Logger.Error("Something very wrong here: Save_Tag_Movie", _tagDB.ToString, "Error")
        ...
        If bWithMovies Then
            ...
            For i = MoviesInTagOld.Count - 1 To 0 Step -1
                For Each movienew In MoviesInTagNew
                    If MoviesInTagOld(i).MainDetails.UniqueIDs.IMDbId = movienew.MainDetails.UniqueIDs.IMDbId Then
    ...
    Public Sub Change_TVEpisode(ByVal episode As DBElement, ByVal listOfEpisodes As List(Of MediaContainers.MainDetails), Optional ByVal batchMode As Boolean = False)
        ...
        Remove_TVEpisode(episode.FileItem.FullPath, False, True)
        ...
        For Each tEpisode As MediaContainers.MainDetails In listOfEpisodes
            ...
            newEpisode.MainDetails.FileInfo = episode.MainDetails.FileInfo
    ...
    ''' <param name="bBatchMode">Is the function already part of a transaction?</param>
    ''' <param name="bToDisk">Create NFO and Images</param>
    Public Function Save_TVEpisode(ByVal dbElement As DBElement, ByVal bBatchMode As Boolean, ByVal bToNFO As Boolean, ByVal bToDisk As Boolean, ByVal bDoSeasonCheck As Boolean, ByVal bDoSync As Boolean, Optional ByVal bForceIsNewFlag As Boolean = False, Optional ByVal forceFileCleanup As Boolean = False) As DBElement
        If dbElement.MainDetails Is Nothing Then Return dbElement
        ...
        RemoveMissingEpisode(dbElement.ShowID, dbElement.MainDetails.Season, dbElement.MainDetails.Episode)
        ...
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
            ...
            'DateAdded
            Try
                If Not Master.eSettings.TVEpisode.SourceSettings.DateAddedIgnoreNfo AndAlso dbElement.MainDetails.DateAddedSpecified Then
                    Dim DateTimeAdded As Date = Date.ParseExact(dbElement.MainDetails.DateAdded, "yyyy-MM-dd HH:mm:ss", Globalization.CultureInfo.InvariantCulture)
                    par_dateAdded.Value = Functions.ConvertToUnixTimestamp(DateTimeAdded)
                Else
            ...
            If Not dbElement.IDSpecified Then
                ...
                Using rdrTVEp As SQLiteDataReader = sqlCommand.ExecuteReader()
                    ...
                    Else
                        _Logger.Error("Something very wrong here: Save_TVEpisode", dbElement.ToString, "Error")
                        dbElement.ID = -1
                        Return dbElement
                        Exit Function
                    End If
            ...
            If dbElement.IDSpecified Then
                ...
                'Guest Stars
                SetGuestStarsForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.GuestStars)
                ...
                If bDoSeasonCheck Then
                    Using SQLSeasonCheck As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                        SQLSeasonCheck.CommandText = String.Format("SELECT idSeason FROM season WHERE idShow = {0} AND Season = {1};", dbElement.ShowID, dbElement.MainDetails.Season)
        ...
        If dbElement.FileItemSpecified AndAlso bDoSync Then
            AddonsManager.Instance.RunGeneric(Enums.AddonEventType.Sync_TVEpisode, Nothing, Nothing, False, dbElement)
        End If
    ...
    Public Function Save_TVSeason(ByRef dbElement As DBElement, ByVal bBatchMode As Boolean, ByVal bToDisk As Boolean, ByVal bDoSync As Boolean, Optional ByVal forceFileCleanup As Boolean = False) As DBElement
        If dbElement.MainDetails Is Nothing Then Return dbElement
        ...
        If Not doesExist Then
            Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                ...
                par_marked.Value = dbElement.IsMarked
                par_new.Value = True
                par_aired.Value = dbElement.MainDetails.Aired
                par_plot.Value = dbElement.MainDetails.Plot
        ...
        If bDoSync Then
            AddonsManager.Instance.RunGeneric(Enums.AddonEventType.Sync_TVSeason, Nothing, Nothing, False, dbElement)
        End If
    ...
    ''' <param name="bBatchMode">Is the function already part of a transaction?</param>
    ''' <param name="bToDisk">Create NFO and Images</param>
    Public Function Save_TVShow(ByRef dbElement As DBElement, ByVal bBatchMode As Boolean, ByVal bToNFO As Boolean, ByVal bToDisk As Boolean, ByVal bWithEpisodes As Boolean, Optional ByVal forceFileCleanup As Boolean = False) As DBElement
        If dbElement.MainDetails Is Nothing Then Return dbElement
        ...
        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
            ...
            'DateAdded
            Try
                If Not Master.eSettings.TVShow.SourceSettings.DateAddedIgnoreNfo AndAlso dbElement.MainDetails.DateAddedSpecified Then
                    Dim DateTimeAdded As Date = Date.ParseExact(dbElement.MainDetails.DateAdded, "yyyy-MM-dd HH:mm:ss", Globalization.CultureInfo.InvariantCulture)
                    par_dateAdded.Value = Functions.ConvertToUnixTimestamp(DateTimeAdded)
                Else
                    par_dateAdded.Value = If(Not dbElement.IDSpecified, Functions.ConvertToUnixTimestamp(Date.Now), dbElement.DateAdded)
            ...
            If Not dbElement.IDSpecified Then
                ...
                Using rdrTVShow As SQLiteDataReader = sqlCommand.ExecuteReader()
                    ...
                    Else
                        _Logger.Error("Something very wrong here: Save_TVShow", dbElement.ToString, "Error")
                        dbElement.ID = -1
                        dbElement.ShowID = dbElement.ID
                        Return dbElement
                        Exit Function
            ...
            If dbElement.IDSpecified Then
                ...
                'Creators
                SetCreatorsForItem(dbElement.ID, dbElement.ContentType, dbElement.MainDetails.Creators)
        ...
        If dbElement.SeasonsSpecified Then
            ...
            Next
            Remove_Invalid_TVSeasons(dbElement.Seasons, dbElement.ID, True)
        End If
        ...
        If bWithEpisodes AndAlso dbElement.EpisodesSpecified Then
            ...
            Next
            Remove_Invalid_TVEpisodes(dbElement.Episodes, dbElement.ID, True)
        End If

        'delete empty seasons after saving all known episodes
        Remove_Empty_TVSeasons(dbElement.ID, True)

        If Not bBatchMode Then SQLtransaction.Commit()

        AddonsManager.Instance.RunGeneric(Enums.AddonEventType.Sync_TVShow, Nothing, Nothing, False, dbElement)
                        ...
                        Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                            ...
                            Using sqlReader As SQLiteDataReader = sqlCommand.ExecuteReader()
                                If sqlReader.HasRows Then
                                    ...
                                    bIsNewSet = False
                                    NFO.Save(dbElement, False) 'to save the "new" SetName and/or SetPlot
                                Else
                                    bIsNewSet = True
                                End If
                            End Using
    ...
    Private Sub SetTVShowLinksForMovie(ByVal dbElement As DBElement)
        ...
        For Each strTitle In dbElement.MainDetails.ShowLinks
...
#Region "Database upgrade Methods"
            ...
            For i As Integer = Args.currVersion To Args.newVersion - 1
                ...
                For Each Trans In _cmds.transaction
                    ...
                    Using sqlTransaction As SQLiteTransaction = _MyvideosDBConn.BeginTransaction()
                        For Each _cmd As Containers.CommandsTransactionCommand In Trans.command
                            If _cmd.type = "DB" Then
                                Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                                    ...
                                    Try
                                        _Logger.Info(String.Concat(Trans.name, ":   ", _cmd.description))
                                        sqlCommand.ExecuteNonQuery()
                                    Catch ex As Exception
                                        TransOk = False
                                        _Logger.Error(New StackFrame().GetMethod().Name, ex, Trans.name, _cmd.description)
                                        Exit For
                                    End Try
                                End Using
                            End If
                        Next
                        If TransOk Then
                            _Logger.Trace(String.Format("Transaction {0} Commit Done", Trans.name))
                            sqlTransaction.Commit()
                            ' Housekeeping - consolidate and pack database using vacuum command http://www.sqlite.org/lang_vacuum.html
                            Using sqlCommand As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                                sqlCommand.CommandText = "VACUUM;"
                            ...
                            End Using
                        Else
                            _Logger.Trace(New StackFrame().GetMethod().Name, String.Format("Transaction {0} RollBack", Trans.name))
                    ...
                    If _cmd.type = "DB" Then
                        Using SQLnotransaction As SQLiteCommand = _MyvideosDBConn.CreateCommand()
                            ...
                            Try
                                ...
                                End Using
                            Catch ex As Exception
                                _Logger.Info(New StackFrame().GetMethod().Name, ex, SQLnotransaction, _cmd.description, _cmd.execute)
                            End Try
                        End Using
                    End If
                Next
                ...
                Select Case Args.currVersion
                    Case Is < 47
                        ...
                        If MessageBox.Show("Ember now saves the resolution of all images in the database. Do you want to scan all images (all sources has to be mountet for this)?", "Get resolution of images", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                            Patch47_art(True)
                        End If
                        _Logger.Info("Set main tab sorting to defaults (needed for New default lists)")
    ...
    Private Sub bwPatchDB_ProgressChanged(ByVal sender As Object, ByVal e As System.ComponentModel.ProgressChangedEventArgs) Handles bwPatchDB.ProgressChanged
        If e.ProgressPercentage = -1 Then
            _Logger.Info(e.UserState.ToString)
                    ...
                    If Not DBNull.Value.Equals(SQLreader("url")) AndAlso Not String.IsNullOrEmpty(SQLreader("url").ToString) Then
                        ...
                        If File.Exists(strValue) Then
                            Try
                                ...
                                End Using
                            Catch ex As Exception
                                _Logger.Error(String.Format("Can't read image (maybe broken): ""{0}""", strValue))
                            End Try
                        End If
                    End If
                End While
...
#Region "Properties"
        ...
        Public ReadOnly Property ThemeSpecified() As Boolean
            Get
                Return (Theme.ThemeOriginal IsNot Nothing AndAlso Theme.ThemeOriginal.HasMemoryStream) OrElse Theme.LocalFilePathSpecified
            End Get
        End Property
        ...
        Public ReadOnly Property TrailerSpecified() As Boolean
            Get
                Return (Trailer.TrailerOriginal IsNot Nothing AndAlso Trailer.TrailerOriginal.HasMemoryStream) OrElse Trailer.LocalFilePathSpecified
            End Get
        End Property
        ''' <summary>
