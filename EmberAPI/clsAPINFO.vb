...
Public Class NFO
    ...
    Public Shared Function GetNfoPath(ByVal sPath As String, ByVal isSingle As Boolean) As String
        ...
        Dim nPath As String = String.Empty 'NFO path
        Dim fileName As String = Path.GetFileNameWithoutExtension(sPath)
        Dim fileNameStack As String = StringUtils.CleanStackingMarkers(Path.GetFileNameWithoutExtension(sPath))
        Dim filePath As String = Path.Combine(Directory.GetParent(sPath).FullName, fileName)
        Dim filePathStack As String = Path.Combine(Directory.GetParent(sPath).FullName, fileNameStack)
        Dim fileParPath As String = Directory.GetParent(filePath).FullName

        '*************** XBMC Frodo settings ***************
        If Master.eSettings.UseFrodo AndAlso Master.eSettings.NFOFrodo Then
        ...
        '****************** YAMJ settings *****************
        If Master.eSettings.UseYAMJ AndAlso Master.eSettings.NFOYAMJ Then
            'YAMJ VIDEO_TS
            If FileUtils.Common.isVideoTS(sPath) Then
                ...
                'YAMJ BDMV folder
            ElseIf FileUtils.Common.isBDRip(sPath) Then
                nPath = String.Concat(Directory.GetParent(Directory.GetParent(fileParPath).FullName).FullName, Path.DirectorySeparatorChar, Directory.GetParent(Directory.GetParent(fileParPath).FullName).Name, ".nfo")
        ...
        If Master.eSettings.UseExpert Then
            ...
            If Not String.IsNullOrEmpty(Master.eSettings.NFOExpertVTS) AndAlso FileUtils.Common.isVideoTS(sPath) Then
                For Each a In Master.eSettings.NFOExpertVTS.Split(New String() {","}, StringSplitOptions.RemoveEmptyEntries)
                    If Master.eSettings.UseBaseDirectoryExpertVTS Then
                        nPath = Path.Combine(Directory.GetParent(fileParPath).FullName, a.Replace("<filename>", fileName))
                    Else
                        nPath = Path.Combine(fileParPath, a.Replace("<filename>", fileName))
                    End If
                    If File.Exists(nPath) Then
                        Return nPath
                    End If
            ...
            End If
            'Expert BDMV folder
            If Not String.IsNullOrEmpty(Master.eSettings.NFOExpertBDMV) AndAlso FileUtils.Common.isBDRip(sPath) Then
                For Each a In Master.eSettings.NFOExpertBDMV.Split(New String() {","}, StringSplitOptions.RemoveEmptyEntries)
                    If Master.eSettings.UseBaseDirectoryExpertBDMV Then
                        nPath = Path.Combine(Directory.GetParent(Directory.GetParent(fileParPath).FullName).FullName, a.Replace("<filename>", fileName))
                    Else
                        nPath = Path.Combine(fileParPath, a.Replace("<filename>", fileName))
                    End If
                    If File.Exists(nPath) Then
            ...
            If Not String.IsNullOrEmpty(Master.eSettings.NFOExpertSingle) AndAlso isSingle Then
                ...
                ElseIf Master.eSettings.StackExpertSingle Then
                    For Each a In Master.eSettings.NFOExpertSingle.Split(New String() {","}, StringSplitOptions.RemoveEmptyEntries)
                        nPath = Path.Combine(fileParPath, a.Replace("<filename>", fileNameStack))
                        If File.Exists(nPath) Then
                            Return nPath
                        End If
            ...
            End If
            'Expert multi folder
            If Not String.IsNullOrEmpty(Master.eSettings.NFOExpertMulti) AndAlso Not isSingle Then
    ...
    Public Shared Sub SaveMovieToNFO(ByRef movieToSave As Structures.DBMovie)
        ...
        Try
            ...
            If Not String.IsNullOrEmpty(movieToSave.Filename) Then
                ...
                Dim fileName As String = Path.GetFileNameWithoutExtension(movieToSave.Filename)
                Dim fileNameStack As String = StringUtils.CleanStackingMarkers(Path.GetFileNameWithoutExtension(movieToSave.Filename))
                Dim filePath As String = Path.Combine(Directory.GetParent(movieToSave.Filename).FullName, fileName)
                Dim filePathStack As String = Path.Combine(Directory.GetParent(movieToSave.Filename).FullName, fileNameStack)
                Dim fileParPath As String = Directory.GetParent(filePath).FullName

                '*************** XBMC Frodo settings ***************
                If Master.eSettings.UseFrodo Then
                    If Master.eSettings.NFOFrodo Then
                ...
                If Master.eSettings.UseEden Then
                    If Master.eSettings.NFOEden Then
                        If FileUtils.Common.isVideoTS(movieToSave.Filename) Then
                            nPath = String.Concat(filePath, ".nfo")
                        ElseIf FileUtils.Common.isBDRip(movieToSave.Filename) Then
                            nPath = Path.Combine(Directory.GetParent(fileParPath).FullName, "index.nfo")
                ...
                If Master.eSettings.UseYAMJ Then
                    If Master.eSettings.NFOYAMJ Then
                        ...
                        ElseIf FileUtils.Common.isBDRip(movieToSave.Filename) Then
                            nPath = String.Concat(Path.Combine(Directory.GetParent(Directory.GetParent(fileParPath).FullName).FullName, Directory.GetParent(Directory.GetParent(fileParPath).FullName).Name), ".nfo")
                        Else
                            nPath = String.Concat(filePath, ".nfo")
                        End If
                ...
                '***************** Expert settings ****************
                If Master.eSettings.UseExpert Then
                    If Not String.IsNullOrEmpty(Master.eSettings.NFOExpertBDMV) OrElse Not String.IsNullOrEmpty(Master.eSettings.NFOExpertMulti) OrElse _
                        Not String.IsNullOrEmpty(Master.eSettings.NFOExpertSingle) OrElse Not String.IsNullOrEmpty(Master.eSettings.NFOExpertVTS) Then
                        If FileUtils.Common.isVideoTS(movieToSave.Filename) Then
                            For Each a In Master.eSettings.NFOExpertVTS.Split(New String() {","}, StringSplitOptions.RemoveEmptyEntries)
                                If Master.eSettings.UseBaseDirectoryExpertVTS Then
                                    nPath = Path.Combine(Directory.GetParent(fileParPath).FullName, a.Replace("<filename>", fileName))
