        ...
        Public Sub SaveAllImages(ByRef dbElement As Database.DBElement, ByVal forceFileCleanup As Boolean)
            ...
            Select Case tContentType
                ...
                Case Enums.ContentType.Movieset
                    ...
                    'MovieSet Banner
                    If Banner.LoadAndCache(tContentType, True) Then
                        If dbElement.MainDetails.Title_HasChanged Then Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainBanner, True)
                        Banner.LocalFilePath = Banner.ImageOriginal.Save(dbElement, Enums.ModifierType.MainBanner)
                    Else
                        Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainBanner, dbElement.MainDetails.Title_HasChanged)
                        Banner = New Image
                    End If

                    'MovieSet ClearArt
                    If ClearArt.LoadAndCache(tContentType, True) Then
                        If dbElement.MainDetails.Title_HasChanged Then Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainClearArt, True)
                        ClearArt.LocalFilePath = ClearArt.ImageOriginal.Save(dbElement, Enums.ModifierType.MainClearArt)
                    Else
                        Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainClearArt, dbElement.MainDetails.Title_HasChanged)
                        ClearArt = New Image
                    End If

                    'MovieSet ClearLogo
                    If ClearLogo.LoadAndCache(tContentType, True) Then
                        If dbElement.MainDetails.Title_HasChanged Then Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainClearLogo, True)
                        ClearLogo.LocalFilePath = ClearLogo.ImageOriginal.Save(dbElement, Enums.ModifierType.MainClearLogo)
                    Else
                        Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainClearLogo, dbElement.MainDetails.Title_HasChanged)
                        ClearLogo = New Image
                    End If

                    'MovieSet DiscArt
                    If DiscArt.LoadAndCache(tContentType, True) Then
                        If dbElement.MainDetails.Title_HasChanged Then Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainDiscArt, True)
                        DiscArt.LocalFilePath = DiscArt.ImageOriginal.Save(dbElement, Enums.ModifierType.MainDiscArt)
                    Else
                        Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainDiscArt, dbElement.MainDetails.Title_HasChanged)
                        DiscArt = New Image
                    End If

                    'MovieSet Fanart
                    If Fanart.LoadAndCache(tContentType, True) Then
                        If dbElement.MainDetails.Title_HasChanged Then Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainFanart, True)
                        Fanart.LocalFilePath = Fanart.ImageOriginal.Save(dbElement, Enums.ModifierType.MainFanart)
                    Else
                        Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainFanart, dbElement.MainDetails.Title_HasChanged)
                        Fanart = New Image
                    End If

                    'MovieSet KeyArt
                    If KeyArt.LoadAndCache(tContentType, True) Then
                        If dbElement.MainDetails.Title_HasChanged Then Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainKeyArt, True)
                        KeyArt.LocalFilePath = KeyArt.ImageOriginal.Save(dbElement, Enums.ModifierType.MainKeyArt)
                    Else
                        Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainKeyArt, dbElement.MainDetails.Title_HasChanged)
                        KeyArt = New Image
                    End If

                    'MovieSet Landscape
                    If Landscape.LoadAndCache(tContentType, True) Then
                        If dbElement.MainDetails.Title_HasChanged Then Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainLandscape, True)
                        Landscape.LocalFilePath = Landscape.ImageOriginal.Save(dbElement, Enums.ModifierType.MainLandscape)
                    Else
                        Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainLandscape, dbElement.MainDetails.Title_HasChanged)
                        Landscape = New Image
                    End If

                    'MovieSet Poster
                    If Poster.LoadAndCache(tContentType, True) Then
                        If dbElement.MainDetails.Title_HasChanged Then Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainPoster, True)
                        Poster.LocalFilePath = Poster.ImageOriginal.Save(dbElement, Enums.ModifierType.MainPoster)
                    Else
                        Images.Delete_MovieSet(dbElement, Enums.ModifierType.MainPoster, dbElement.MainDetails.Title_HasChanged)
                        Poster = New Image
                    End If
...
#Region "Properties"
        ...
        Public Property Year() As Integer
            Get
                Return StringUtils.GetYearFromString(Premiered)
            End Get
            Set(value As Integer)
                If Not PremieredSpecified Then
                    Premiered = value.ToString
                End If
            End Set
        End Property
                        ...
                        Select Case xNode.NodeType
                            Case XmlNodeType.Element
                                Select Case xNode.Name
                                    ...
                                    Case "tmdb"
                                        nSetInfo.TMDbId = CInt(xNode.InnerText)
                                End Select
            ...
            If _sets.Count > 0 AndAlso _sets.Item(0).TitleSpecified Then
                ...
                If Master.eSettings.Movie.DataSettings.Collection.SaveExtendedInformation Then
                    ...
                    If firstSet.TMDbIdSpecified Then
                        ...
                        Dim NodeTMDB_Text As XmlText = XmlDoc.CreateTextNode(firstSet.TMDbId.ToString)
...
#Region "Properties"
        ...
        Public ReadOnly Property ListTitle() As String
            Get
                Return StringUtils.SortTokens(DBMovie.MainDetails.Title)
            End Get
        End Property
...
#Region "Properties"
        ...
        Public Property IsGuestStar() As Boolean = False
...
#Region "Methods"
        ...
        Public Sub CreateCachePaths(ByRef tDBElement As Database.DBElement)
            ...
            Select Case tDBElement.ContentType
                Case Enums.ContentType.Movie
                    strID = tDBElement.MainDetails.UniqueIDs.IMDbId
                    If String.IsNullOrEmpty(strID) Then
                        strID = "Unknown"
                    End If
                    strPath = Path.Combine(Master.TempPath, String.Concat("Movies", Path.DirectorySeparatorChar, strID))
                Case Enums.ContentType.Movieset
                    intID = tDBElement.MainDetails.UniqueIDs.TMDbId
                    If intID = -1 Then
                        strID = "Unknown"
                    Else
                        strID = intID.ToString
                    End If
                    strPath = Path.Combine(Master.TempPath, String.Concat("MovieSets", Path.DirectorySeparatorChar, strID))
                Case Enums.ContentType.TV, Enums.ContentType.TVEpisode, Enums.ContentType.TVSeason, Enums.ContentType.TVShow
                    intID = tDBElement.MainDetails.UniqueIDs.TVDbId
                    If intID = -1 Then
                        strID = "Unknown"
                    Else
                        strID = intID.ToString
                    End If
                    strPath = Path.Combine(Master.TempPath, String.Concat("Shows", Path.DirectorySeparatorChar, strID))
                Case Else
            ...
            For Each tImg As Image In EpisodeFanarts
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("episodefanarts", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("episodefanarts\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In EpisodePosters
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("episodeposters", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("episodeposters\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In MainBanners
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("mainbanners", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("mainbanners\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In MainCharacterArts
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("maincharacterarts", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("maincharacterarts\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In MainClearArts
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("maincleararts", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("maincleararts\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In MainClearLogos
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("mainclearlogos", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("mainclearlogos\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In MainDiscArts
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("maindiscarts", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("maindiscarts\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In MainFanarts
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("mainfanarts", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("mainfanarts\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In MainKeyArts
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("mainposters", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("mainposters\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In MainLandscapes
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("mainlandscapes", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("mainlandscapes\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next
            ...
            For Each tImg As Image In SeasonBanners
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("seasonbanners", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("seasonbanners\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In SeasonFanarts
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("seasonfanarts", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("seasonfanarts\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In SeasonLandscapes
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("seasonlandscapes", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("seasonlandscapes\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next

            For Each tImg As Image In SeasonPosters
                tImg.CacheOriginalPath = Path.Combine(strPath, String.Concat("seasonposters", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                If Not String.IsNullOrEmpty(tImg.URLThumb) Then
                    tImg.CacheThumbPath = Path.Combine(strPath, String.Concat("seasonposters\_thumbs", Path.DirectorySeparatorChar, Path.GetFileName(tImg.URLOriginal)))
                End If
            Next
        End Sub
...
#Region "Properties"
        ...
        Public Property Seasons() As List(Of MainDetails) = New List(Of MainDetails)
...
#Region "Properties"
        ...
        <XmlIgnore()>
        Public Property TMDbId() As Integer = -1
                ...
                Select Case tContentType
                    Case Enums.ContentType.Movie
                        If .Theme.LoadAndCache() Then
                            Themes.Delete(tDBElement, ForceFileCleanup)
                            .Theme.LocalFilePath = .Theme.ThemeOriginal.Save(tDBElement)
                        Else
                            Themes.Delete(tDBElement, ForceFileCleanup)
                            .Theme = New Theme
                        End If
                ...
                Select Case tContentType
                    Case Enums.ContentType.Movie
                        ...
                        If .Trailer.LoadAndCache() Then
                            Trailers.Delete(tDBElement, ForceFileCleanup)
                            .Trailer.LocalFilePath = .Trailer.TrailerOriginal.Save(tDBElement)
                        Else
                            Trailers.Delete(tDBElement, ForceFileCleanup)
                            .Trailer = New Trailer
                        End If
