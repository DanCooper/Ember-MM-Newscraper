...
#Region "Methods"
            ...
            Try
                ...
                Else
                    ...
                    If Not Master.eSettings.TVShow.SourceSettings.OverWriteNfo Then
                        RenameNonConfNFO(nfoPath, False)
                    End If
                End If
    ...
    Private Shared Sub ManipulateData(ByRef data As MediaContainers.MainDetails, ByVal type As Enums.ContentType)
        ...
        With nSettings
            'GuestStars as Actors
            If data.GuestStarsSpecified AndAlso .GuestStars.AddToActors AndAlso Not .Actors.Locked Then
                For Each person In data.GuestStars
                    person.IsGuestStar = True
                Next
                data.Actors.AddRange(data.GuestStars)
                'run the limit filter again
                Information.FilterCountLimit(nSettings.Actors.Limit, data.Actors)
                'reorder again
                Information.ReorderPersons(data.Actors)
            End If

            'Rating/Votes
            If data.RatingsSpecified Then
                Dim nDefaultRating = data.Ratings.FirstOrDefault(Function(f) f.IsDefault)
                If nDefaultRating IsNot Nothing Then
                    data.Rating = nDefaultRating.Value.ToString
                    data.Votes = nDefaultRating.Votes.ToString
                    'digit grouping symbol for Votes count
                    If Master.eSettings.Options.Global.DigitGrpSymbolVotesEnabled Then
                        If data.VotesSpecified Then
                            Dim vote As String = Double.Parse(data.Votes, Globalization.CultureInfo.InvariantCulture).ToString("N0", Globalization.CultureInfo.CurrentCulture)
                            If vote IsNot Nothing Then data.Votes = vote
                        End If
                    End If
                End If
            End If
    ...
    Private Shared Sub RenameNonConfNFO(ByVal nfoPath As String, ByVal isChecked As Boolean)
    ...
    Public Shared Sub Save(ByRef dbElement As Database.DBElement, ByVal forceFileCleanup As Boolean)
        Dim bIsExisting As Boolean
        Dim nSettings As SourceSettings = Nothing
        Try
            Try
                ...
                Select Case dbElement.ContentType
                    Case Enums.ContentType.Movie
                        bIsExisting = dbElement.FileItemSpecified
                        nSettings = Master.eSettings.Movie.SourceSettings
                        AddonsManager.Instance.RunGeneric(Enums.AddonEventType.OnNFOSave_Movie, params, doContinue, False)
                    Case Enums.ContentType.Movieset
                        bIsExisting = dbElement.MainDetails.TitleSpecified
                        nSettings = Master.eSettings.Movieset.SourceSettings
                        AddonsManager.Instance.RunGeneric(Enums.AddonEventType.OnNFOSave_Movieset, params, doContinue, False)
                    Case Enums.ContentType.TVEpisode
                        bIsExisting = dbElement.FileItemSpecified
                        nSettings = Master.eSettings.TVEpisode.SourceSettings
                        AddonsManager.Instance.RunGeneric(Enums.AddonEventType.OnNFOSave_TVEpisode, params, doContinue, False)
                    Case Enums.ContentType.TVSeason
                        nSettings = Master.eSettings.TVSeason.SourceSettings
                        AddonsManager.Instance.RunGeneric(Enums.AddonEventType.OnNFOSave_TVSeason, params, doContinue, False)
                    Case Enums.ContentType.TVShow
                        bIsExisting = dbElement.ShowPathSpecified
                        nSettings = Master.eSettings.TVShow.SourceSettings
            ...
            If bIsExisting AndAlso nSettings IsNot Nothing Then
                ...
                'Create a clone of MediaContainer to prevent changes on database data that only needed for NFO
                Dim nData As MediaContainers.MainDetails = CType(dbElement.MainDetails.CloneDeep, MediaContainers.MainDetails)

                'Manipulate data by global and content type settings
                ManipulateData(nData, dbElement.ContentType)
                ...
                For Each a In FileUtils.FileNames.GetFileNames(dbElement, Enums.ModifierType.MainNFO)
                    If Not nSettings.OverWriteNfo Then
