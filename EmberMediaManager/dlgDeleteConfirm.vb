        ...
        Try
            With tvFiles
                ...
                Using SQLtransaction As SQLite.SQLiteTransaction = Master.DB.MyVideosDBConn.BeginTransaction() 'Only on Batch Mode
                    For Each ItemParentNode As TreeNode In .Nodes
                        Select Case _deltype
                            Case Enums.DelType.Movies
                                Master.DB.DeleteFromDB_Movie(Convert.ToInt64(ItemParentNode.Tag), True)
                            Case Enums.DelType.Shows
                                Master.DB.DeleteFromDB_TVShow(Convert.ToInt64(ItemParentNode.Tag), True)
                            Case Enums.DelType.Seasons
                                tPair = DirectCast(ItemParentNode.Tag, KeyValuePair(Of Long, Long))
                            Case Enums.DelType.Episodes
                                Master.DB.DeleteFromDB_TVEpisode(Convert.ToInt64(ItemParentNode.Tag), False, True, True)
                        End Select
                                ...
                                If node.Checked Then
                                    Select Case node.ImageKey
                                        Case "FOLDER"
                                            ...
                                            If oDir.Exists Then
                                                oDir.Delete(True)
                                                If _deltype = Enums.DelType.Seasons Then Master.DB.DeleteFromDB_TVSeason(tPair.Value, Convert.ToInt32(tPair.Key), True)
                                                Exit For
                                            End If
                                        ...
                                        Case "FILE"
                                            ...
                                            If oFile.Exists Then
                                                oFile.Delete()
                                                If _deltype = Enums.DelType.Seasons AndAlso Master.eSettings.FileSystemValidExts.Contains(IO.Path.GetExtension(node.Tag.ToString)) Then Master.DB.DeleteFromDB_TVEpisode(node.Tag.ToString, True, True)
                                            End If
                                    End Select
                    ...
                    Next
                    If _deltype = Enums.DelType.Seasons OrElse _deltype = Enums.DelType.Episodes Then Master.DB.DeleteFromDB_EmptyTVSeasons(True)
                ...
                Select Case _deltype
                    ...
                    Case Enums.DelType.Shows
                        For Each ShowID As Long In ItemsToDelete.Keys
                            ...
                            Dim tShow As Database.DBElement = Master.DB.LoadFromDB_TVSeason(ShowID, False, False)
                    ...
                    Case Enums.DelType.Seasons
                        ...
                        Using SQLDelCommand As SQLite.SQLiteCommand = Master.DB.MyVideosDBConn.CreateCommand()
                            For Each Season As KeyValuePair(Of Long, Long) In ItemsToDelete
                                ...
                                Dim tSeason As Database.DBElement = Master.DB.LoadFromDB_TVSeason(Season.Value, Convert.ToInt32(Season.Key), True, True)
                    ...
                    Case Enums.DelType.Episodes
                        ...
                        Using SQLCommand As SQLite.SQLiteCommand = Master.DB.MyVideosDBConn.CreateCommand()
                            For Each Ep As Long In ItemsToDelete.Keys
                                ...
                                Dim tEpisode As Database.DBElement = Master.DB.LoadFromDB_TVEpisode(Ep, True)
