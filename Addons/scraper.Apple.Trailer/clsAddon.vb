        ...
        Try
            If Not String.IsNullOrEmpty(OriginalTitle) Then
                ...
                If sjson = String.Empty OrElse sjson.Length < 50 Then
                    ...
                    _Logger.Debug("[" & OriginalTitle & "] Apple Trailer - Movie NOT found on http://trailers.apple.com/ - URL: " & String.Format("http://trailers.apple.com/trailers/home/scripts/quickfind.php?q={0}", searchtitle))
                    ...
                    If Not String.IsNullOrEmpty(sHtml) Then
                        ...
                        If Not trailerWEBURLResults Is Nothing AndAlso trailerWEBURLResults.Count > 0 Then
                                ...
                                If Not String.IsNullOrEmpty(TrailerWEBPageURL) Then
                                    ...
                                    If TrailerWEBPageURL.Contains("/") Then
                                        ...
                                        If (OriginalTitle.Length <= 5 AndAlso compareresult = 0) OrElse (OriginalTitle.Length > 5 AndAlso compareresult <= 2) Then
                                            _Logger.Debug("[" & OriginalTitle & "] Apple Trailer - Google: Movie found! URL: " & TrailerWEBPageURL)
                                            Exit For
                                        Else
                                            _Logger.Debug("[" & OriginalTitle & "] Apple Trailer - Google: wrong result! URL: " & TrailerWEBPageURL)
                                            TrailerWEBPageURL = String.Empty
                                        End If
                                    End If
                                End If
                            Next
                        Else
                            _Logger.Debug("[" & OriginalTitle & "] Apple Trailer - Movie NOT found on http://trailers.apple.com/ using google! URL: " & SearchURL)
                        End If
                    Else
                        _Logger.Debug("[" & OriginalTitle & "] Apple Trailer - Movie NOT found on http://trailers.apple.com/ using google! URL: " & SearchURL)
                    End If
                ...
                Else
                    ...
                    If Not jsonresult Is Nothing AndAlso Not jsonresult.error AndAlso Not jsonresult.results Is Nothing AndAlso jsonresult.results.Count > 0 Then
                        ...
                        If String.IsNullOrEmpty(jsonresult.results(0).location) Then
                            _Logger.Debug("[" & OriginalTitle & "] Apple Trailer - Movie NOT found on http://trailers.apple.com/ - URL: " & TrailerWEBPageURL)
                        Else
                            _Logger.Debug("[" & OriginalTitle & "] Apple Trailer - Movie found on http://trailers.apple.com/ - URL: " & TrailerWEBPageURL)
                        End If
                    Else
                        _Logger.Debug("[" & OriginalTitle & "] Apple Trailer - Movie NOT found on http://trailers.apple.com/ - URL: " & String.Format("http://trailers.apple.com/trailers/home/scripts/quickfind.php?q={0}", searchtitle))
                    End If
                ...
                If Not String.IsNullOrEmpty(TrailerWEBPageURL) Then
                    ...
                    _Logger.Info("[" & OriginalTitle & "] Apple Trailer - Movie found! Download URL: " & TrailerWEBPageURL)
                    ...
                    If Not zResult Is Nothing AndAlso zResult.Count > 0 Then
                        For ctr As Integer = 0 To zResult.Count - 1
                            ...
                            nTrailerList.Add(New MediaContainers.Trailer With {.URLVideoStream = zResult.Item(ctr).Groups("LINK").Value, .Title = zResult.Item(ctr).Groups("TITLE").Value, .Duration = zResult.Item(ctr).Groups("DURATION").Value})
                        Next

                        For Each trailer In nTrailerList
                    ...
                    End If
                Else
                    _Logger.Info("[" & OriginalTitle & "] Apple Trailer - Movie NOT found")
                End If
            Else
                _Logger.Warn("[" & OriginalTitle & "] Apple Trailer - Original title is empty, no scraping of trailers possible!")
            End If

            'log scraperesults for user
            If nTrailerList.Count < 1 Then
                _Logger.Info("[" & OriginalTitle & "] Apple Trailer - NO trailer scraped!")
            Else
                _Logger.Info("[" & OriginalTitle & "] Apple Trailer - Scraped " & nTrailerList.Count & " trailer!")
            End If
