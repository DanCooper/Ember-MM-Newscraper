...
#Region "Methods"
            ...
            Select Case iType
                Case Enums.ScrapeType.AllAsk, Enums.ScrapeType.FilterAsk, Enums.ScrapeType.MarkedAsk, Enums.ScrapeType.MissingAsk, Enums.ScrapeType.NewAsk, Enums.ScrapeType.SelectedAsk, Enums.ScrapeType.SingleField
                    If r.Matches.Count = 1 Then
                        Return GetTVShowInfo(r.Matches.Item(0).UniqueIDs.TVDbId, ScrapeModifiers, FilteredOptions, False)
                    Else
                        Using dlgSearch As New dlgSearchResults(_AddonSettings, Me)
                            If dlgSearch.ShowDialog(r, sShowName, oDBTV.ShowPath) = DialogResult.OK Then
                                If Not String.IsNullOrEmpty(dlgSearch.Result.UniqueIDs.TVDbId) Then
                                    Return GetTVShowInfo(dlgSearch.Result.UniqueIDs.TVDbId, ScrapeModifiers, FilteredOptions, False)
                                End If
                            End If
                        End Using
                    End If
        ...
        Private Function SearchTVShowByName(ByVal sShow As String) As SearchResults
            ...
            Shows = _TVDBApi.GetSeriesByName(sShow, _AddonSettings.Language, _TVDBMirror).Result
        ...
        Private Async Function GetFullSeriesById(ByVal tvdbID As Integer) As Task(Of TVDB.Model.SeriesDetails)
            Dim Result As TVDB.Model.SeriesDetails = Await _TVDBApi.GetFullSeriesById(tvdbID, _AddonSettings.Language, _TVDBMirror)
            Return Result
        End Function
        ''' <summary>
        ''' 
