...
#Region "Methods"
    ...
    Private Sub btnDown_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDown.Click
        Dim order As Integer = Addons.Instance.Theme_Scrapers_TV.FirstOrDefault(Function(p) p.AssemblyFileName = Addon._AssemblyName).Order
        If order < Addons.Instance.Theme_Scrapers_TV.Count - 1 Then
            Addons.Instance.Theme_Scrapers_TV.FirstOrDefault(Function(p) p.Order = order + 1).Order = order
            Addons.Instance.Theme_Scrapers_TV.FirstOrDefault(Function(p) p.AssemblyFileName = Addon._AssemblyName).Order = order + 1
    ...
    Sub orderChanged()
        Dim order As Integer = Addons.Instance.Theme_Scrapers_TV.FirstOrDefault(Function(p) p.AssemblyFileName = Addon._AssemblyName).Order
        If Addons.Instance.Theme_Scrapers_TV.Count > 1 Then
            btnDown.Enabled = (order < Addons.Instance.Theme_Scrapers_TV.Count - 1)
