...
#Region "Fields"
    ...
    'current edited host
    Private _currentHost As Generic.Host = Nothing
...
#Region "Properties"
    ...
    Public Property Result As Generic.Host
        Get
            Return _currentHost
        End Get
        Set(value As Generic.Host)
            _currentHost = value
        End Set
    End Property
...
#Region "Constructors"
    ...
    Sub New(ByVal Host As Generic.Host)
...
#Region "Methods"
    ...
    Private Sub PopulateHostSources()
        ...
        For Each moviesources As Database.DBSource In Master.DB.Load_AllSources_Movie
            LocalSources.Add(moviesources.Path, Enums.ContentType.Movie)
        Next
        For Each showsources As Database.DBSource In Master.DB.Load_AllSources_TVShow
        ...
        If dgvHostSources.Rows.Count > 0 Then
            For i = 0 To dgvHostSources.Rows.Count - 1
                If Not String.IsNullOrEmpty(CStr(dgvHostSources.Rows(i).Cells(0).Value)) AndAlso Not String.IsNullOrEmpty(CStr(dgvHostSources.Rows(i).Cells(1).Value)) Then
                    Dim nSource As New Generic.Source
