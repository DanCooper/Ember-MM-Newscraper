...
#Region "Methods"
    ...
    Private Sub btnDown_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDown.Click
        Dim order As Integer = Addons.Instance.Image_Scrapers_Movieset.FirstOrDefault(Function(p) p.AssemblyFileName = Addon._AssemblyName).Order
        If order < Addons.Instance.Image_Scrapers_Movieset.Count - 1 Then
            Addons.Instance.Image_Scrapers_Movieset.FirstOrDefault(Function(p) p.Order = order + 1).Order = order
            Addons.Instance.Image_Scrapers_Movieset.FirstOrDefault(Function(p) p.AssemblyFileName = Addon._AssemblyName).Order = order + 1
    ...
    Private Sub chkScrapeFanart_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) Handles chkScrapeFanart.CheckedChanged
        RaiseEvent ModuleSettingsChanged()
    End Sub

    Private Sub chkScrapePoster_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) Handles chkScrapePoster.CheckedChanged
        RaiseEvent ModuleSettingsChanged()
    End Sub

    Sub orderChanged()
        Dim order As Integer = Addons.Instance.Image_Scrapers_Movieset.FirstOrDefault(Function(p) p.AssemblyFileName = Addon._AssemblyName).Order
        If Addons.Instance.Image_Scrapers_Movieset.Count > 1 Then
            btnDown.Enabled = (order < Addons.Instance.Image_Scrapers_Movieset.Count - 1)
