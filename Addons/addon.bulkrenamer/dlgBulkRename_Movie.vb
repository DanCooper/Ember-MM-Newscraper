...
#Region "Fields"
    ...
    Private _CancelRename As Boolean = False
    Private _Columnsize(9) As Integer
    Private _DlgHelpTips As dlgHelpTips
    Private _DoneRename As Boolean = False
    Private _FFRenamer As Renamer
    Private _IsLoaded As Boolean = False
    Private _RunOnce As Boolean = True

    Public _FilterMovies As String = String.Empty
    Public _FilterMoviesSearch As String = String.Empty
    Public _FilterMoviesType As String = String.Empty
    Public _ListMovies As String = String.Empty
...
#Region "Methods"
    ...
    Private Sub bwDoRename_DoWork(ByVal sender As Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bwDoRename.DoWork
        _FFRenamer.DoRename_Movies(AddressOf ShowProgressRename)
    End Sub
    ...
    Private Sub bwLoadInfo_DoWork(ByVal sender As Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles bwLoadInfo.DoWork
        Try
            ...
            If Not String.IsNullOrEmpty(_FilterMoviesSearch) AndAlso Not String.IsNullOrEmpty(_FilterMoviesType) Then
                Select Case _FilterMoviesType
                    Case Master.eLang.GetString(100, "Actor")
                        dbFilter = String.Concat("idMovie IN (SELECT actorlinkmovie.idMovie FROM actorlinkmovie ",
                                                 "WHERE idActor IN (SELECT actors.idActor FROM actors WHERE strActor LIKE '%", _FilterMoviesSearch, "%'))")
                    Case Master.eLang.GetString(233, "Role")
                        dbFilter = String.Concat("idMovie IN (SELECT idMovie FROM actorlinkmovie WHERE strRole LIKE '%", _FilterMoviesSearch, "%')")
                End Select
                hasFilter = True
            End If

            If Not String.IsNullOrEmpty(_FilterMovies) Then
                If hasFilter Then
                    dbFilter = String.Concat("(", _FilterMovies, ") AND ", dbFilter).Trim
                Else
                    dbFilter = String.Concat(_FilterMovies).Trim
                    hasFilter = True
                End If
            End If
            ...
            Using SQLNewcommand As SQLite.SQLiteCommand = Master.DB.MyVideosDBConn.CreateCommand()
                ...
                If Not hasFilter Then
                    SQLNewcommand.CommandText = String.Format("SELECT COUNT(idMovie) AS mcount FROM '{0}';", _ListMovies)
                Else
                    SQLNewcommand.CommandText = String.Format("SELECT COUNT(idMovie) AS mcount FROM '{0}' WHERE {1};", _ListMovies, dbFilter)
                End If
                ...
                End Using
                If Not hasFilter Then
                    SQLNewcommand.CommandText = String.Format("SELECT NfoPath, idMovie FROM '{0}' ORDER BY Title ASC;", _ListMovies)
                Else
                    SQLNewcommand.CommandText = String.Format("SELECT NfoPath, idMovie FROM '{0}' WHERE {1} ORDER BY Title ASC;", _ListMovies, dbFilter)
                End If
                                ...
                                If Not DBNull.Value.Equals(SQLreader("NfoPath")) AndAlso Not DBNull.Value.Equals(SQLreader("idMovie")) Then
                                    ...
                                    If Not String.IsNullOrEmpty(_tmpPath) Then
                                        ...
                                        If _currMovie.IDSpecified AndAlso _currMovie.FileItemSpecified Then
                                            ...
                                            bwLoadInfo.ReportProgress(iProg, StringUtils.SortTokens(_currMovie.MainDetails.Title))
                                        End If
                                    End If
                                End If
    ...
    Private Sub chkRenamedOnly_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) Handles chkRenamedOnly.CheckedChanged
        If chkRenamedOnly.Checked Then
            _BSMovies.Filter = "IsRenamed = True AND IsLocked = False"
        Else
    ...
    Private Sub cmsMovieList_Opening(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmsMovieList.Opening
        Dim count As Integer = _FFRenamer.GetCountAll_Movies
        Dim lockcount As Integer = _FFRenamer.GetCountLocked_Movies
        If count > 0 Then
            If lockcount > 0 Then
    ...
    Private Sub DoProcess()
        ...
        Invoke(Sta)
        _FFRenamer.ProccessFiles_Movies(txtFolderPattern.Text, txtFilePattern.Text, txtFolderPatternNotSingle.Text)
        Invoke(Fin)
    End Sub
    ...
    Private Sub Rename_Button_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Rename_Button.Click
        ...
        pbCompile.Style = ProgressBarStyle.Continuous
        pbCompile.Maximum = _FFRenamer.GetCountMax_Movies
        pbCompile.Value = 0
        Application.DoEvents()
        'Start worker
    ...
    Sub setLock(ByVal lock As Boolean)
        For Each row As DataGridViewRow In dgvMoviesList.SelectedRows
            _FFRenamer.SetIsLocked_Movies(row.Cells(1).Value.ToString, row.Cells(2).Value.ToString, lock)
    ...
    Sub setLockAll(ByVal lock As Boolean)
        Try
            _FFRenamer.SetIsLocked_Movies(String.Empty, String.Empty, False)
    ...
    Private Function ShowProgressRename(ByVal mov As String, ByVal iProg As Integer) As Boolean
        ...
        If _CancelRename Then Return False
        Return True
    End Function
    ...
    Private Sub Simulate()
        Try
            With dgvMoviesList
                ...
                End If
                _BSMovies.DataSource = _FFRenamer.GetTable_Movies
                .DataSource = _BSMovies
    ...
    Private Sub tmrSimul_Tick(ByVal sender As Object, ByVal e As EventArgs) Handles tmrSimul.Tick
        Try
            'TODO: Need to make simulate thread safe
    ...
    Sub LoadHelpTips()
        If _DlgHelpTips Is Nothing OrElse _DlgHelpTips.IsDisposed Then
            _DlgHelpTips = New dlgHelpTips
        End If
        Dim s As String = String.Format(Master.eLang.GetString(262, "$1 = First Letter of the Title{0}$2 = Aired date (episodes only){0}$3 = ShortStereoMode{0}$4 = StereoMode{0}$5 = Collection List Title{0}$A = Audio Channels{0}$B = Base Path{0}$C = Director{0}$D = Directory{0}$E = Sort Title{0}$F = File Name{0}$G = Genre (Follow with a space, dot or hyphen, comma to change separator){0}$H = Video Codec{0}$I = IMDB ID{0}$J = Audio Codec{0}$K#.S? = #Padding (0-9), Season Separator (. or _ or x), Season Prefix{0}$L = List Title{0}$M = MPAA{0}$N = Collection Name{0}$O = OriginalTitle{0}$OO = OriginalTitle if different from Title{0}$P = Rating{0}$Q#.E? = #Padding (0-9), Episode Separator (. or _ or x), Episode Prefix{0}$R = Resolution{0}$S = Video Source{0}$T = Title{0}$U = Country (Follow with a space, dot, comma or hyphen to change separator){0}$V = 3D (If Multiview > 1){0}$W#.S?#.E? = #Padding (0-9), Seasons Separator (. or _), Season Prefix, #Padding (0-9), Episode Separator (. or _ or x), Episode Prefix{0}$Y = Year{0}$X. (Replace Space with .){0}$Z = Show Title{0}{{}} = Optional{0}$?aaa?bbb? = Replace aaa with bbb{0}$! = Uppercase first letter in each word{0}$; = Lowercase all letters{0}$- = Remove previous char if next pattern does not have a value{0}$+ = Remove next char if previous pattern does not have a value{0}$^ = Remove previous and next char if next pattern does not have a value"), Environment.NewLine)
        _DlgHelpTips.lblTips.Text = s
        _DlgHelpTips.Width = _DlgHelpTips.lblTips.Width + 5
        _DlgHelpTips.Height = _DlgHelpTips.lblTips.Height + 35
        _DlgHelpTips.Top = Top + 10
        _DlgHelpTips.Left = Right - _DlgHelpTips.Width - 10
        If _DlgHelpTips.Visible Then
            _DlgHelpTips.Hide()
        Else
            _DlgHelpTips.Show(Me)
        End If
    End Sub
