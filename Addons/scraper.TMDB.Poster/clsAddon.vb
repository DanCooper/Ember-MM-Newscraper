...
#Region "Methods"
    ...
    Public Async Function CreateAPI(ByVal AddonSettings As AddonSettings) As Task
        Try
            _Client = New TMDbLib.Client.TMDbClient(AddonSettings.APIKey)
            Await _Client.GetConfigAsync()
            _Client.MaxRetryCount = 2
            _Logger.Trace("[TMDB_Image] [CreateAPI] Client created")
        Catch ex As Exception
            _Logger.Error(String.Format("[TMDB_Image] [CreateAPI] [Error] {0}", ex.Message))
        End Try
    End Function
